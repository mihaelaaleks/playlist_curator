...
def get_recommendation(limit:int, 
    ...
    if limit >= 100:
        raise ValueError("Limit must not exceed 100")    
    response = spotify.recommendations(limit = limit,
...
def get_cleaned_recommendations(limit:int, 
                       spotify: spotipy.Spotify,
                       t_acousticness: int,
                       t_danceability: int, 
                       t_energy: int, 
                       t_instrumentalness: int):
    recommendation = get_recommendation(limit=limit,
                                        spotify=spotify,
